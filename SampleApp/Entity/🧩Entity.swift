...
enum 🧩Entity {
    ...
    static func genericBalls() -> [Entity] {
        let vals = (0..<32).map { _ in Self.genericBall() }
        return vals
    }
...
private extension 🧩Entity {
    ...
    private static func genericBall() -> Entity {
        let value = Entity()
        // value.name = 🧩Name.genericBall
        value.components.set([CollisionComponent(shapes: [.generateSphere(radius: 0.04)]),
                              🧩Model.genericBall(),
